 ...
 */
fun parseCatFromTokens(env: Environment, tokens: List<String>, inputNodes: List<AstNode>?): Cat? {
    if (tokens.isEmpty() || tokens[0] != "cat") {
        return null
    }
    inputNodes?.let { return Cat(env, inputNodes) }
    return Cat(env, tokens.subList(1, tokens.size).toTextNodes(true))
}
 ...
 * its output printed.
 */
class Cat(private val env: Environment, args: List<AstNode>) : Command(args, "cat", true, false) {

    override fun consumeArgument(arg: AstNode) {
        val input = arg.getOutput()
        if (arg.isArgument()) {
            val inputInDir = env.inCurrentWorkDir(input)
            appendToOutput(File(inputInDir).inputStream().bufferedReader().use { it.readText() } + "\n")
        } else {
